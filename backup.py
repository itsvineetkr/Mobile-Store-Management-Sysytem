...
"""User details are in this way:
1. 1000 admin 
2. 1001 username1
3. 1002 username2
Passwords are only in creds and can be accessed by user id
"""
...
def add_item():
    ...
    .format(itemid,comp,model,ram,storage,scrsize,resolution,scrtype,pro,battery,rcamera,fcamera,price,info)
    cur.execute(q1)
...
def see_item():
    ...
    d=cur.fetchall()
    header=['item id','company','model','ram','storage','screen size','resolution','screen type','processor','battery','rcamera','fcamera','price','additional info']
...
def update_spec():
    ...
    col=input('Enter coloumn name (itemid,company,model,ram,storage,scrsize,resolution,scrtype,pro,battery,rcamera,fcamera,price,info)\n')
    n=input('Enter new data: ')
    if col in ['itemid','company','model','resolution','scrtype','pro','rcamera','fcamera','review']:
...
def change_pass(userid, password):
    ...
    if oldpass==password:
        ...
        while True:    
            if newpass==repass:
                q="update creds set userpass='{}' where userid={};".format(newpass,userid)
                cur.execute(q)
                db.commit()
                break
            else:
...
def purchase_history(username,userid=1000):
    ...
    if username=='admin':
        ...
        cur.execute('select itemname,usold,date from purchasehistory where userid={}'.format(userid))
        d=cur.fetchall()
...
def append_wishlist(username, itemid):
    ...
    d=cur.fetchall()[0]
    cur2.execute("insert into {} values('{}','{}','{}',{})".format(username,d[0],d[1],d[2],0))
    db2.commit()
...
def see_wishlist(username):
    ...
    d=cur2.fetchall()
    header=['Item id','Company','Model','Purchased']
    print(tabulate(d,headers=header,tablefmt='pretty'))
...
def purchase(username, userid, itemid, units):
    ...
    if int(units)<=int(d[0][1]):
        ...
        cur.execute("select * from items where itemid='{}'".format(itemid))
        d=cur.fetchall()[0]
        q1="insert into purchasehistory values('{}','{}',{},{},'{}')".format(itemid,d[1]+' '+d[2],userid,units,date)
        cur.execute(q1)
        ...
        for p,q,r,s in d2:
            l.append(p)
        if itemid in l:
            q3="update {} set purchase=purchase+{} where itemid='{}'".format(username,units,itemid)
            cur2.execute(q3)
        else:
            cur2.execute("insert into {} values('{}','{}','{}',{})".format(username,d[0],d[1],d[2],units))
        db.commit()
        db2.commit()
        price=int(units)*int(d[-2])
        l=[['You have successfully purchased {} {} on date {}'.format(d[1],d[2],date)],
...
def user_page(userid, username, password):
    ...
    There is a while loop which will run till user types "exit". It is a menu driven code which will ask for 
    user's choice (numbers from 1 to 6 assigned for different things) and will perform various tasks.
    ...
    """
    print('------------------------------------------------------\n')
    print(tabulate([['Hello {} senpai!'.format(username.capitalize())]],tablefmt='rounded_outline'))
    login_history(userid,username)
    c=''
    while c!='exit':
        print()
        t=[['To see items details press 1'],
...
def admin_page(password):
    ...
    while c!='exit':
        ...
        if c=='1':
            ...
            d=cur.fetchall()[0][0]
            print(tabulate([['Revenue generated till now is: {} INR'.format(d)]],tablefmt='pretty'))
        elif c=='10':
...
def login_page():
    ...
    if choice=='admin':
        ...
        if username in l:
            print()
            print(tabulate([['User name already exists! try again.']],tablefmt='rounded_outline'))
            print()
            login_page()
            return None
        else:
            ...
            if rpass==password:
                cur.execute("insert into creds values({},'{}')".format(userid,password))
                cur.execute("insert into users values({},'{}')".format(userid,username))
                cur2.execute("create table {}(itemid varchar(5) primary key,company varchar(20),model varchar(20),purchase int)".format(username))
                db.commit()
                print(tabulate([['User created successfully']],tablefmt='rounded_outline'))
            else:
                print('Passwords do not match try again.')
                login_page()
        print(tabulate([['If you want to go to login page then press 1'],['If you want to exit then press 2']],tablefmt='rounded_outline'))
        ...
        if len(d)!=0:
            ...
            if password==passwd:
                ...
                print()
                print(tabulate([['You entered wrong password try again later!']],tablefmt='rounded_outline'))
                print()
                login_page()
